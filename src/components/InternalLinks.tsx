...
export function InternalLinks({ type, limit = 6, currentYear }: InternalLinksProps) {
  ...
  const renderHijriConvertLinks = () => {
    ...
    return years.slice(0, limit).map(year => (
      <Link 
        key={year}
        to={`/convert/hijri-to-gregorian/${year}`}
        className="block p-3 rounded-lg bg-secondary/50 hover:bg-secondary transition-colors text-sm"
      >
  ...
  return (
    <section className="mt-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        ...
        {type === 'all' && (
          <>
            <Link to="/date/today" className="block p-3 rounded-lg bg-primary/10 hover:bg-primary/20 transition-colors text-sm font-medium">
              تاريخ اليوم
            </Link>
            <Link to="/how-old-am-i/hijri" className="block p-3 rounded-lg bg-primary/10 hover:bg-primary/20 transition-colors text-sm font-medium">
              حساب العمر بالهجري
            </Link>
            <Link to="/how-old-am-i/gregorian" className="block p-3 rounded-lg bg-primary/10 hover:bg-primary/20 transition-colors text-sm font-medium">
              حساب العمر بالميلادي
            </Link>
            <Link to={`/calendar/${hijri.year}`} className="block p-3 rounded-lg bg-primary/10 hover:bg-primary/20 transition-colors text-sm font-medium">
