    ...
    <PageLayout>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex justify-between items-center mb-8">
          <Link 
            to={`/white-days/${prevMonth.month.slug}-${prevMonth.year}`}
            className="text-primary hover:underline"
          >
            ← {prevMonth.month.name} {prevMonth.year}
          </Link>
          <Link 
            to={`/white-days/${nextMonth.month.slug}-${nextMonth.year}`}
            className="text-primary hover:underline"
          >
        ...
        <div className="bg-secondary/30 rounded-xl p-6 mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <Link to={`/calendar/${year}`} className="text-primary hover:underline text-sm">التقويم الهجري</Link>
            <Link to={`/hijri-month/start/${monthSlug}-${year}`} className="text-primary hover:underline text-sm">بداية {monthName}</Link>
            <Link to={`/events/ramadan-${year}`} className="text-primary hover:underline text-sm">موعد رمضان</Link>
          </div>
        </div>
