    ...
    <PageLayout>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex justify-between items-center mb-8">
          <Link 
            to={`/salary/date/${prevMonth.month.slug}-${prevMonth.year}`}
            className="text-primary hover:underline"
          >
            ← {prevMonth.month.name} {prevMonth.year}
          </Link>
          <Link 
            to={`/salary/date/${nextMonth.month.slug}-${nextMonth.year}`}
            className="text-primary hover:underline"
          >
        ...
        <div className="bg-secondary/30 rounded-xl p-6 mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <Link to="/how-old-am-i/hijri" className="text-primary hover:underline text-sm">حساب العمر</Link>
            <Link to={`/calendar/${hijriDate.year}`} className="text-primary hover:underline text-sm">التقويم الهجري</Link>
            <Link to={`/white-days/muharram-${hijriDate.year}`} className="text-primary hover:underline text-sm">الأيام البيض</Link>
          </div>
        </div>
