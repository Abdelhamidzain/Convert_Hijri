    ...
    <PageLayout>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
                ...
                {HIJRI_MONTHS.map((m) => {
                  ...
                  return (
                    <tr key={m.number} className={`border-b border-border/50 ${isCurrent ? 'bg-primary/10' : ''}`}>
                      <td className="p-3">
                        <Link 
                          to={`/hijri-month/start/${m.slug}-${year}`}
        ...
        <div className="flex justify-between items-center mb-8">
          <Link 
            to={`/hijri-month/start/${prevMonth.month.slug}-${prevMonth.year}`}
            className="text-primary hover:underline"
          >
            ← {prevMonth.month.name} {prevMonth.year}
          </Link>
          <Link 
            to={`/hijri-month/start/${nextMonth.month.slug}-${nextMonth.year}`}
            className="text-primary hover:underline"
          >
        ...
        <div className="bg-secondary/30 rounded-xl p-6 mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <Link to={`/calendar/${year}`} className="text-primary hover:underline text-sm">التقويم الهجري {year}</Link>
            <Link to={`/white-days/${monthSlug}-${year}`} className="text-primary hover:underline text-sm">الأيام البيض</Link>
            <Link to="/date/today" className="text-primary hover:underline text-sm">تاريخ اليوم</Link>
            <Link to={`/convert/hijri-to-gregorian/${year}`} className="text-primary hover:underline text-sm">تحويل التاريخ</Link>
          </div>
        </div>
